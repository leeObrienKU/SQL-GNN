...
def main():
    ...
    parser.add_argument('--lr', type=float, default=0.001,
                        help='Learning rate')
    parser.add_argument('--lr_decay_type', type=str, default='exponential', choices=['none','exponential','step'],
                        help='Learning rate decay scheduler type')
    parser.add_argument('--lr_decay_gamma', type=float, default=0.95,
                        help='LR decay factor (gamma) for exponential/step schedulers')
    ...
    # Thresholding for binary attrition
    ...
    parser.add_argument('--wandb_api_key', type=str, default=None,
                        help='W&B API key (or set WANDB_API_KEY env).')
    ...
    if args.wandb and _WANDB_AVAILABLE:
        ...
        logger.wandb_run = wandb.init(project=args.wandb_project,
                                      entity=args.wandb_entity,
                                      config={},
                                      name=f"{args.model}-{args.task}-{logger.experiment_id}")
    ...
    if logger.wandb_run is not None:
        try:
            ...
            logger.wandb_run.log({
                ...
                "plots/confusion_matrix_normalized": wandb.Image(os.path.join(out_dir, "confusion_matrix_normalized.png")),
            })
            # Also log raw confusion matrix
            logger.wandb_run.log({"confusion_matrix/raw": cm})
        except Exception:
            pass
