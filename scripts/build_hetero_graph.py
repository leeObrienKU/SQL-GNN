...
# --- Build real edges (with bidirectional logic)
print("\n🔗      Building real edges...")
edge_count = 0
...
for src_table, info in schema.items():
    for fk in info.get("foreign_keys", []):
        ...
        if edge_src:
            ...
            # --- Add reverse edge
            reverse_edge_type = (ref_table, f"{ref_table}_to_{src_table}", src_table)
            data[reverse_edge_type].edge_index = edge_index.flip(0)
            edge_count += 1

            print(f"✅ Built edge: {src_table} → {ref_table} [{edge_index.shape[1]} edges]")
            print(f"↩️  Built reverse edge: {ref_table} → {src_table} [{edge_index.shape[1]} edges]")
        else:
